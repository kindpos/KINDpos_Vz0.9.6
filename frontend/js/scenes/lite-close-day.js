...
// Placeholder closed checks for tip adjustment
const CLOSED_CHECKS = [
  { id: 'C-091', total: 42.00, tip: 0 },
  { id: 'C-092', total: 18.50, tip: 0 },
  { id: 'C-093', total: 27.75, tip: 0 },
  { id: 'C-094', total: 55.00, tip: 0 },
  { id: 'C-095', total: 13.25, tip: 0 },
];

registerLiteScene('lite-close-day', {
  onEnter(el) {
    const checks = CLOSED_CHECKS.map(c => ({ ...c }));
      ...
      el.innerHTML = `
        <div style="display:grid;grid-template-columns:1fr 280px;gap:8px;flex:1;min-height:0;">
          ...
          <div style="${PANEL}display:flex;flex-direction:column;overflow:hidden;">
            <div style="${HEADER_BAR}">TIP ADJUSTMENT <span style="opacity:0.5;font-size:11px;">PLACEHOLDER</span></div>
          ...
          <div style="display:flex;flex-direction:column;gap:8px;">
            <div style="${PANEL}display:flex;flex-direction:column;">
              <div style="${HEADER_BAR}">CASH EXPECTED <span style="opacity:0.5;font-size:11px;">PLACEHOLDER</span></div>
            ...
            <div style="${PANEL}padding:12px;display:flex;flex-direction:column;gap:8px;">
              <div style="font-family:var(--fb);font-size:12px;opacity:0.4;text-align:center;">PLACEHOLDER — Not connected to batch processing</div>

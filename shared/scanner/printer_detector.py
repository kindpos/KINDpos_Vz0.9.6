...
class PrinterDiscovery:
    ...
    # Ports that indicate a hardware device
    HARDWARE_PORTS = {
        9100: "escpos",   # ESC/POS raw TCP socket (PRIMARY)
        8443: "payment",  # Dejavoo SPIn payment device (SSL)
        9000: "payment",  # Dejavoo SPIn payment device (Non-SSL)
    ...
    def scan_network(
        ...
        Args:
            ...
            methods: Discovery methods to use. Options:
                ...
                If None, uses ["port_scan", "mdns", "usb"].
        """
        if methods is None:
            methods = ["port_scan", "mdns", "usb"]
    ...
    def _nmap_scan(self, network_cidr: str) -> List[DiscoveredPrinter]:
        ...
        discovered = []
        nm = nmap.PortScanner()
        port_list = ",".join(str(p) for p in self.HARDWARE_PORTS.keys())
        ...
        try:
            ...
            nm.scan(hosts=network_cidr, arguments=f"-p {port_list} -T4 -Pn --open")
            ...
            for idx, host in enumerate(nm.all_hosts()):
                ...
                if "tcp" in nm[host]:
                    for port, port_info in nm[host]["tcp"].items():
                        if port_info["state"] == "open" and port in self.HARDWARE_PORTS:
                ...
                if 9100 in open_ports:
                    protocol = "escpos"
                elif 8443 in open_ports or 9000 in open_ports:
                    protocol = "spin"
                ...
                # Log all hosts for debugging discovery misses
                logger.info(f"[SCANNER] DEBUG: Emitted {host} to UI")
    ...
    def _socket_scan(self, network_cidr: str) -> List[DiscoveredPrinter]:
        ...
        for idx, host in enumerate(hosts):
            ...
            open_ports = []
            for port in self.HARDWARE_PORTS.keys():
    ...
    def _mdns_discovery(self) -> List[DiscoveredPrinter]:
        ...
        class PrinterListener:
            ...
            def add_service(self, zeroconf, type, name):
                ...
                if info:
                    ...
                    manufacturer = props.get('mfg') or props.get('manufacturer')
                    model = props.get('mdl') or props.get('model')
                    
                    # Try to infer manufacturer from name if not in props
                    if not manufacturer:
                        if 'Epson' in name: manufacturer = 'Epson'
                        elif 'Star' in name: manufacturer = 'Star Micronics'

                    printer = DiscoveredPrinter(
                        ip_address=ip,
                        mac_address="unknown", # mDNS doesn't always provide MAC
                        hostname=info.server.rstrip('.'),
                        open_ports=[info.port],
                        manufacturer=manufacturer,
                        model=model,
                        protocol="escpos" if info.port == 9100 else "ipp",
                    ...
                    self_orchestrator._emit("host_found", {
                        "ip": ip,
                        "name": name,
                        "manufacturer": manufacturer,
                        "model": model,
        ...
        # Common printer mDNS service types
        service_types = ["_printer._tcp.local.", "_ipp._tcp.local.", "_pdl-datastream._tcp.local."]
        
        self._emit("progress", {"message": "Browsing for mDNS services...", "style": "normal"})
        
        browsers = [ServiceBrowser(zeroconf, t, listener) for t in service_types]
    ...
    def _usb_discovery(self) -> List[DiscoveredPrinter]:
        ...
        try:
            ...
            self._emit("progress", {"message": "Scanning USB bus for printers...", "style": "normal"})

            try:
                # This is a bit tricky because python-escpos usually expects vendor/product IDs
                # We can use pyusb to find potential printers
                # PRINTER class is 7
                devices = usb.core.find(find_all=True, bDeviceClass=7)
                
                # Also check interfaces if bDeviceClass is 0
                if not devices:
                    def is_printer(dev):
                        for cfg in dev:
                            if usb.util.find_descriptor(cfg, bInterfaceClass=7) is not None:
                                return True
                        return False
                    devices = usb.core.find(find_all=True, custom_match=is_printer)
            except NoBackendError:
                logger.warning("[SCANNER] No USB backend (libusb) available. Skipping USB discovery.")
                return []
            ...
            for dev in devices:
                try:
                    ...
                    manufacturer = f"USB Vendor {hex(dev.idVendor)}"
                    product = f"USB Product {hex(dev.idProduct)}"
                    serial = "unknown"

                path = f"usb://{hex(dev.idVendor)}:{hex(dev.idProduct)}"
                
                printer = DiscoveredPrinter(
                    ip_address=path, # Using path as identifier for USB
                    mac_address=serial or "unknown",
                    manufacturer=manufacturer,
                    model=product,
                    protocol="escpos",
            ...
            logger.error(f"[SCANNER] USB discovery error: {e}")
